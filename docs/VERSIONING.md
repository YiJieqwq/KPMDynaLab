...
RPC API:   10
Event ABI: 3
KPM:       0.8.8-gesture-observer-test
CLI:       0.8.8-gesture-observer-test
```
...
Compatibility policy is deliberately strict during development: Event ABI must match exactly. RPC API can later support a negotiated range, but the current KPM requires CLI API 10 and exposes API 10.
